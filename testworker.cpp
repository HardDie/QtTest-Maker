...
void TestWorker::SetTestMode(testmode_t testmode) {
    ...
    case TESTMODE_MIX:
        qDebug() << "Set Mix mode";
        break;
    case TESTMODE_TYPE_QA:
        qDebug() << "Set Type QA mode";
        break;
    case TESTMODE_TYPE_AQ:
        qDebug() << "Set Type AQ mode";
        break;
    case TESTMODE_TYPE_MIX:
        qDebug() << "Set Type Mix mode";
        break;
    case TESTMODE_NOTSET:
        qDebug() << "ERROR: Set not valid mode";
        break;
    }
    _testmode = testmode;
}
...
bool TestWorker::GetString(QString &resultString) {
    switch (_testmode) {
    case TESTMODE_QA:
        return GetQAString(resultString);
    case TESTMODE_AQ:
        return GetAQString(resultString);
    case TESTMODE_MIX:
    ...
    case TESTMODE_TYPE_MIX:
        break;
    case TESTMODE_NOTSET:
        qDebug() << "ERROR: Not valid test mode";
        break;
    }
    return false;
}
...
QString TestWorker::GetIndexString() {
    if (ValidateIndex()) {
        if (_testmode == TESTMODE_MIX || _testmode == TESTMODE_TYPE_MIX) {
            if (_mix_phase == MIX_PHASE_FIRST) {
                return QString::number(_currentIndex + 1) + "/" + QString::number(_listData.length() * 2);
            } else if (_mix_phase == MIX_PHASE_SECOND) {
                return QString::number(_listData.length() + _currentIndex + 1) + "/" + QString::number(_listData.length() * 2);
            }
        } else {
            return QString::number(_currentIndex + 1) + "/" + QString::number(_listData.length());
        }
    }
    return QString();
}
...
bool TestWorker::GetMixString(QString &resultString) {
    ...
    case MIX_PHASE_FIRST:
        if (ValidateIndex() == false) {
            ...
            _currentIndex = 0;
            Shuffle();
            // Если просто выйти в этот момент, то в строке будет пусто т.к. мы только сменили режим
            // Чтобы этого не произошло, мы снова вызываем эту же функцию только уже с новым режимом
            return GetMixString(resultString);
        }

        if (_listData[_currentIndex]._flag == FLAGSTATE_NONE) {
            _listData[_currentIndex]._flag = rand() % 2 ? FLAGSTATE_AQ : FLAGSTATE_QA;
        }
...
bool TestWorker::Next() {
    ...
    if (_currentIndex < 0 ||
        _currentIndex >= _listData.length()) {
        return false;
    }
