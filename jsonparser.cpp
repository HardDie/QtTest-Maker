...
QList<struct Question> JsonParser::ParseJsonToQuestionsList(QByteArray rawFile) {
    QList<Question> list;
...
QByteArray JsonParser::FormatJsonFileFromQuestionList(const QList<struct Question> &list) {
    if (list.length() == 0) {
    ...
    foreach (struct Question question, list) {
        QJsonObject jsonVal;
        jsonVal.insert("ans", QJsonValue::fromVariant(question._answer));
        jsonVal.insert("que", QJsonValue::fromVariant(question._question));
